...
'''
    if request.method == "POST":
        ...
        un = request.form.get("regname")
        email = request.form.get("regemail")
        pw = request.form.get("pswd")
        print(un, email, pw)
        ...
        elif not re.match(r'[A-Za-z0-9]+', un):
            mg = 'name must contain only character and number'
        else:
            insert_sql = 'INSERT INTO users (USERNAME,FIRST_NAME,LAST_NAME,EMAIL,PASSWORD) VALUES (?,?,?,?,?)'
            pstmt = ibm_db.prepare(conn, insert_sql)
            ibm_db.bind_param(pstmt, 1, un)
...
'''    msg = ''
    ...
    if request.method == 'POST':
        un = request.form.get("logname")
        pd = request.form.get("logpswd")
        print(un, pd)
        sql = "SELECT * FROM users WHERE USERNAME =? AND PASSWORD=?"
        stmt = ibm_db.prepare(conn, sql)
